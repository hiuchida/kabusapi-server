...
public class RankingApiController implements RankingApi {
    ...
    public ResponseEntity<RankingDefaultResponse> rankingGet(@Parameter(in = ParameterIn.HEADER, description = "トークン発行メソッドで取得した文字列" ,required=true,schema=@Schema()) @RequestHeader(value="X-API-KEY", required=true) String X_API_KEY
...
,@NotNull @Parameter(in = ParameterIn.QUERY, description = "市場<br> ※業種別値上がり率・値下がり率に市場を指定しても無視されます <table>   <thead>       <tr>           <th>定義値</th>           <th>説明</th>       </tr>   </thead>   <tbody>       <tr>           <td>ALL</td>           <td>全市場（デフォルト）</td>       </tr>       <tr>           <td>T</td>           <td>東証全体</td>       </tr>       <tr>           <td>TP</td>           <td>東証プライム</td>       </tr>       <tr>           <td>TS</td>           <td>東証スタンダード</td>       </tr>       <tr>           <td>TG</td>           <td>グロース250</td>       </tr>       <tr>           <td>M</td>           <td>名証</td>       </tr>       <tr>           <td>FK</td>           <td>福証</td>       </tr>       <tr>           <td>S</td>           <td>札証</td>       </tr>   </tbody> </table>" ,required=true,schema=@Schema(allowableValues={ "ALL", "T", "TP", "TS", "TG", "M", "FK", "S" }
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<RankingDefaultResponse>(objectMapper.readValue("{\n  \"Type\": 1,\n  \"ExchangeDivision\": \"ALL\",\n  \"Ranking\": [\n    {\n      \"No\": 1,\n      \"Trend\": \"1\",\n      \"AverageRanking\": 999,\n      \"Symbol\": \"3358\",\n      \"SymbolName\": \"ﾜｲｴｽﾌｰﾄﾞ\",\n      \"CurrentPrice\": 297,\n      \"ChangeRatio\": 71,\n      \"ChangePercentage\": 34.41,\n      \"CurrentPriceTime\": 900,\n      \"TradingVolume\": 1629.5,\n      \"Turnover\": 458.575,\n      \"ExchangeName\": \"東証ス\",\n      \"CategoryName\": \"小売業\"\n    },\n    {\n      \"No\": 2,\n      \"Trend\": \"1\",\n      \"AverageRanking\": 999,\n      \"Symbol\": \"7014\",\n      \"SymbolName\": \"名村造\",\n      \"CurrentPrice\": 199,\n      \"ChangeRatio\": 43,\n      \"ChangePercentage\": 27.56,\n      \"CurrentPriceTime\": 900,\n      \"TradingVolume\": 19720,\n      \"Turnover\": 3797.37,\n      \"ExchangeName\": \"東証ス\",\n      \"CategoryName\": \"輸送用機器\"\n    }\n  ]\n}", RankingDefaultResponse.class), HttpStatus.OK);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<RankingDefaultResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<RankingDefaultResponse>(HttpStatus.NOT_IMPLEMENTED);
    }
